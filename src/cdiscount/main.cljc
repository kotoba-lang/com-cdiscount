(ns cdiscount.main
  "Kotodama WASM entrypoint for the Cdiscount clean-room actor (L4) — Clojure port.
  ...
  facts (`cdiscount.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cdiscount")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "cdiscoun_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "cdiscoun_cus"
    :fields [:email :firstName :lastName :phone]               :required [:email :firstName]
    :coerce {}                                 :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "cdiscoun_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"cdiscount.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cdiscount-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cdiscount-compat/ready)
